...
// Leaves the digest the hasher will produce next unchanged, but squeezes
// through a scratch buffer, so it is not safe to call concurrently on one
// Hasher.
func (h *Hasher) Sum256() [32]byte {
	h.init()
	return [32]byte(h.h.Sum(h.h.sum[:0]))
}
